...
def _aggregate_to_bars(df: "pl.DataFrame", bar_sec: int, state_features: str = "rich") -> "pl.DataFrame":
    ...
    bars = bars.with_columns(pl.col("volume").fill_null(0))
...
def _load_parquet_sequences(
    ...
    for sym in symbols:
        for fp in list(base.glob(f"{sym}/{sym}_*.parquet"))[:10]:
            try:
