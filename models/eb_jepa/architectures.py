...
class TimeSeriesEncoder(nn.Module):
    """MLP encoder for time series. Input (B, C, T, 1, 1) -> output (B, D, T, 1, 1)."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """x: [B, C, T, H, W] -> out: [B, D, T, 1, 1]."""
        b, c, t, h, w = x.shape
        x_flat = x.permute(0, 2, 1, 3, 4).reshape(b * t, c)
        out = self.mlp(x_flat)
        out = self.final_ln(out)
        out = out.view(b, t, -1).permute(0, 2, 1).unsqueeze(-1).unsqueeze(-1)
        return out
